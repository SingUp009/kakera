/**
 * Image decode helpers. Runs inside the Web Worker (uses `OffscreenCanvas` /
 * `createImageBitmap`). Produces tightly-packed, non-premultiplied sRGB RGBA8 —
...
/**
 * Decode a tile downscaled so its longest edge is `<= maxEdge`. This bounds
 * WASM linear-memory footprint (source detail beyond the rendered cell size is
 * discarded by the core anyway).
 */
